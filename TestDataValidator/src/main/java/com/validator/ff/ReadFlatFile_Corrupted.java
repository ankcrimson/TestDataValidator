...
public class ReadFlatFile_Corrupted {
	...
	public void readFile(List<ExcellRow> srcRows, String filename,IDOCNode root,boolean ...trim)
	{
		int i=0;
		...
		try{
			...
			while((currLine=ipbr.readLine())!=null)
			{
				...
				{	try{
									...
									String[] sfa1=sr.getXpath().split("/");
									String[] sfa2=sfa1[0].split(":");
									String[] sfa3=sfa2[0].split("\\(");
									int qualStart=Integer.parseInt(sfa3[1]);
									String[] sfa4=sfa2[1].split("\\)");
									int qualLen=Integer.parseInt(sfa4[0]);
							...
							//System.out.println("Updown=>"+updown+"<");
										...
										String[] sfb2=sfa1[1].split(":");
										String[] sfb3=sfb2[0].split("\\(");
										int valStart=Integer.parseInt(sfb3[1]);
										String[] sfb4=sfb2[1].split("\\)");
										int valLen=Integer.parseInt(sfb4[0]);
							...
							//System.out.println("root="+root);
										...
										try{
											...
											int aqualStart=Integer.parseInt(iquals[1]);
											//String[] aquals2=aquals[1].split("=");
											//int aqualLen=Integer.parseInt(aquals2[0]);
											int aqualLen=Integer.parseInt(iquals[2]);
											String aval=currLine.substring(aqualStart, aqualStart+aqualLen);
											attrNode.setValue(aval);
											//attrNode.setXpath(""+aqualStart+":"+aqualLen);
											attrNode.setXpath(iquals[0]+"("+aqualStart+":"+aqualLen+")");
											attrNode.setUid(++i);
											newNode.getChildren().add(attrNode);
											//System.out.println(">>>>>>>>>>>"+newNode.getParent().getChildren());
										}catch(Exception e){JOptionPane.showMessageDialog(null, "Please provide flat file IDoc Qualifier properly for "+sr.getIdocAttributes()+"eg. QualName(146:3)=001");}
