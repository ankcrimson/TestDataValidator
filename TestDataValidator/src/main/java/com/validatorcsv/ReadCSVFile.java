...
public class ReadCSVFile {
	...
	public int getUpdown(String newRowXP, String currRowXP, IDOCNode root)
	{
		...
		if(xlevel>ylevel||currRowXP.trim().length()<1) return 1;
		else return -1;
	...
	public void readFile(boolean hRow,List<ExcellRow> srcRows, String filename,IDOCNode root,String delimiter,boolean ...trim)
	{
		int i=0;
		structure=makeStructure(srcRows);
		LoggingClass.logger.info("Structure ="+structure);
		...
		try{
			...
			BufferedReader ipbr = new BufferedReader (new InputStreamReader (new FileInputStream (ipf), "UTF-8"));
			String currLine="";
			if(hRow)
				ipbr.readLine();//remove Header Row
			while((currLine=ipbr.readLine())!=null)
			{
				if(gettQual()!=null&&gettQual().length()>0)
				{
				currLine=currLine.replaceAll(gettQual(), "");
				}
				List<ExcellRow> currentRowMatches=new ArrayList<ExcellRow>();
				String[] allVals=currLine.split(delimiter);
				...
				{	try{
									...
									if(props.length==5){
										//=================================================
										String qual=allVals[Integer.parseInt(props[1])];//currLine.substring(qualStart, qualStart+qualLen);
										if(props[0].equals(qual))
							...
							//System.out.println("Updown=>"+updown+"<");
										...
										sr.setUsed(true);
										String val=allVals[Integer.parseInt(props[4])];//currLine.substring(valStart,valStart+valLen);
										if(trim[0])
										{
											val=val.trim();
										}
							...
							//System.out.println("root="+root);
										...
										try{
											...
											String[] iquals=sr.getIdocAttributes().split("[\\(]|[\\)]|[:]|[=]");
											//String[] aquals2=sr.getIdocAttributes().split("=");//aquals[1].split("=");
											//int aqualP=Integer.parseInt(aquals2[0]);
											//String aval=allVals[Integer.parseInt(aquals2[0])];//currLine.substring(aqualStart, aqualStart+aqualLen);
											String aval=allVals[Integer.parseInt(iquals[1])];
											attrNode.setValue(aval);
											attrNode.setXpath(iquals[0]+"("+iquals[1]+")");
											attrNode.setUid(++i);
											newNode.getChildren().add(attrNode);
											//System.out.println(">>>>>>>>>>>"+newNode.getParent().getChildren());
										}catch(Exception e){
											LoggingClass.logger.info("Please provide CSV IDoc Qualifier properly for "+sr.getIdocAttributes()+"eg. CCCC(3)=Pay");
											JOptionPane.showMessageDialog(null, "Please provide CSV IDoc Qualifier properly for "+sr.getIdocAttributes()+"eg. CCCC(3)=Pay");}
...
*/									
					...
					LoggingClass.logger.info("Either file is not having equal number of elements in each row or CSV XPath for "+sr.getXpath()+" needs to be provided properly. eg. H1(0):LIFNR(1)   or  LIFNR(1) ");
					//JOptionPane.showMessageDialog(null, "Please provide CSV XPath properly for "+sr.getXpath()+"eg. H1(0):LIFNR(1)   or  LIFNR(1) ");
					}
				catch(NumberFormatException nfe){
					LoggingClass.logger.info("Please provide CSV XPath properly for "+sr.getXpath()+"eg. H1(0):LIFNR(1)   or  LIFNR(1) ");
					JOptionPane.showMessageDialog(null, "Please provide CSV XPath properly for "+sr.getXpath()+"eg. H1(0):LIFNR(1)   or  LIFNR(1) ");}
				}
			}
			ipbr.close();
	...
	public IDOCNode makeStructure(List<ExcellRow> srcRows)
	{
		...
		{
			if(r.getParentXpath()==null||r.getParentXpath().trim().length()<1||r.getParentXpath().equals(r.getXpath()))
			{
		...
		for(ExcellRow r:srcRows)
		{
			if(r.getParentXpath()==null||r.getParentXpath().trim().length()<1)
			{}
			else
			{	
