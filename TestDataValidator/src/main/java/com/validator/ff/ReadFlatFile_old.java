	...
	public void readFile(List<ExcellRow> srcRows, String filename,IDOCNode root,boolean ...trim)
	{
		...
		try{
			...
			while((currLine=ipbr.readLine())!=null)
			{
				...
				}catch(ArrayIndexOutOfBoundsException ae){JOptionPane.showMessageDialog(null, "Please provide flat file XPath properly for "+sr.getXpath()+"eg. 0:7:E2EDK01:146:35");}
				}
			}
			ipbr.close();
			ipfr.close();
		}catch(Exception e){e.printStackTrace();JOptionPane.showMessageDialog(null, "Error in reader");}
	}
